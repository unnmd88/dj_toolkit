...
#                     if f_name == 'fctg':
#                         comparison_operator, val = data.pop(i + 1), data.pop(i + 1)
#                         while val[-1] == ')':
#                             if not val:
#                                 raise ValueError(f'Недопустимое значение функции fctg: {token}')
#                             val = val[:-1]
#                         token = self._validate_fctg(token, comparison_operator, val)
#                     break
#             if token is None:
#                 continue
#
...
#         if len(digit_argument) > 1 or int(digit_argument[0]) > 999 or digit_argument[0].startswith('0'):
